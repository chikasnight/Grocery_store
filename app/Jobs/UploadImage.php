...
class UploadImage implements ShouldQueue
{
    ...
    {
        $disk = $this->grocery->disk;
        Log::info("Disk: " . $disk);
        $imageName = $this->grocery->image;
        ...
        try {
            ...
            // Original
            if (Storage::disk($disk)->put('/uploads/groceries/original/' . $imageName, fopen($original_file, 'r+'))) {
                File::delete($original_file);
            }

            // Large
            if (Storage::disk($disk)->put('/uploads/groceries/large/' . $imageName, fopen($large, 'r+'))) {
                File::delete($large);
            }

            // Thumbnail
            if (Storage::disk($disk)->put('/uploads/groceries/thumbnail/' . $imageName, fopen($thumbnail, 'r+'))) {
